...
# returns a data frame of Lon, Lat, Variable from Slab2 data
#
...
# function to get the average size of the graph made by connecting subfault centers
checkSlabSize = function(fault){
  S = length(fault)
  centroids = matrix(0, nrow=S, ncol=2)
  triangles = list()
  
  for (subfault in 1:S){
    centroids[subfault,1] = fault[[subfault]]$lon
    centroids[subfault,2] = fault[[subfault]]$lat
    triangles[[subfault]] = fault[[subfault]]$corners[,c(1,2)]
  }
  
  centroids = projCSZ(centroids, units="km")
  D = as.matrix(dist(centroids))
  A = computeNeighbours(triangles)
  D2 = A*D
...
shareEdge = function(tri1, tri2) {
  count = 0
  for (i in 1:nrow(tri1)) {
    for (j in 1:nrow(tri2)) {
      if (all(tri1[i, ] == tri2[j, ])) {
        count = count + 1
      }
    }
  }
...
computeNeighbours = function(triangles){
  S = length(triangles)
