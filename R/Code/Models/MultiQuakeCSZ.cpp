...
Type objective_function<Type>::operator() ()
{
  ...
  Type nll3 = 0;     // Initialise the spatial negative log likelihood
  ...
  for(int j=0; j<M; j++){
    ...
    nll4 -= sum(dnorm(thisSubsidence, okadaSubsidence, thisV, true));
  }
  //----------------------------------------------------------------------------
  ...
  REPORT(nll1);   // spde prior
  REPORT(nll2);   // taper prior 
  REPORT(nll3);   // spatial random effects
  REPORT(nll4);   // data contribution
  REPORT(X);      // underlying spatial distribution
  REPORT(rho);    // range
  REPORT(sigma2); // marginal variance
  //----------------------------------------------------------------------------
